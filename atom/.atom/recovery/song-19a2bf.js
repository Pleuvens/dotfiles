...
export default class SongItem extends Component {
    ...
    toggleSong = (event) => {
        ...
        while (!parent.classList.contains('song-item'))
            parent = parent.parentNode;

        if (parent.children.item(1).classList.contains('hidden'))
            parent.children.item(1).classList.remove('hidden');
        else
            parent.children.item(1).classList.add('hidden');
    }
    ...
    displayArray = (array) => {
        ...
        for (const i in array) {
            display.push(<div className="song-input-array-item">{array[i].name}</div>);
        }
        return display;
    }
    ...
    render() {
        ...
        return (
            <div className="song-item">
                <div className="song-item-header" onClick={this.toggleSong}><span>{song.title}</span><span>{this.displayArtists(song)}</span></div>
                <div className="song-item-body hidden">
                    <DynamicInput item={song.url} />
                    <DynamicInput item={song.title} />
                    <DynamicInput item={song.date} />
