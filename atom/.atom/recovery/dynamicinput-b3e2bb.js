...
export default class DynamicInput extends Component {
    ...
    validate = (event) => {
        ...
        console.log(node.children.item(1).value);
        this.hideInput(node.children.item(1), node.children.item(0), buttons);
    }
    ...
    componentDidMount() {
        ...
        if (!item.value) {
            document.querySelector('.dynamic-input-value').classList.add('hidden');
            document.querySelector('.dynamic-input-modify').classList.add('hidden');
        } else {
            document.querySelector('.dynamic-input-input').classList.add('hidden');
            document.querySelector('.dynamic-input-validate').classList.add('hidden');
            document.querySelector('.dynamic-input-cancel').classList.add('hidden');
        }
    }

    render() {
        return (<div className="dynamic-input">
